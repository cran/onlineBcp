...
#' @param theta the probability of occurrence of a change point
#' @param alpha the hyperparameter of posterior distribution
#' @param beta the hyperparameter of posterior distribution
#' @param th_cp threshold level for the posterior distribution of change point
#   (alpha, beta): the hyperparameter of posterior distribution
#   alpha: 1                                                      #
#   beta: (0.5, 1)                                                #
#   th_cp: threshold level for the posterior distribution of      #
#          change point                                           #
#   recomended value 0.5                                          #
#                                                #
###################################################################
...
#' @param xlab the default x-axis label
#' @param ylab the default y-axis label
#' @param ... the plotting parameters passed to plot()
#' @return No return value, called for side effects
#' @export
plot.BayesCP <-  function(x, xlab = "Index", ylab = "x", ...) {
  #print("Do some plots")
  #plot(seq_along(bcp$x), bcp$x, xlab = "Index", ylab = "x")
  bcp <- x   ## change name of paramter x to bcp
  max_ps=array(0, c(dim(bcp$max_p)[1:1],4))
  max_ps[,1]=1:dim(bcp$max_p)[1:1]
  max_ps[,2:4]=bcp$max_p
  n=length(max_ps[,1])
  x <- bcp$x   ## x: raw x
  ...
  xc=split(mean_[,3], mean_[,2])
  yc=split(x, mean_[,2])
  zc=split(mean_[,1], mean_[,2])
  ...
  for (i in 1:length(yc)) {
    lines(xc[[i]], zc[[i]],col="red", lty=1, lwd=2)
  }
...
summary.BayesCP <- function(object, ...){
  ...
  k <- dim(changePoints)[1]

  # if(k == 0) print("There are no change points.")
  # else if(k == 1) {
  #   print("There is 1 change point.")
  #   print(paste0("The change point is at index ", changePoints[1, 2], ", with posterior probability = ", changePoints[1, 1]))
  # }
  # else {
  #   print(paste0("There are ", n, " change points"))
  #   for(i in 1:k){
  #     print(paste0("Change point ", i, " is at index ", changePoints[i, 2], ", with posterior probability = ", changePoints[i, 1]))
  #   }
  # }
